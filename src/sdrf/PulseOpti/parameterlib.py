...
def get_target_spindomain_parameters_list(cube:mri.SpinArray,roi,pulse_function:str,phase,flip=0,roi_offset=[0,0,0]):
    ...
    if pulse_function=='refocusing':
        ...
        _,beta = cube.calculate_target_spindomain_excitation(
            flip=flip,phase=phase,roi=roi,roi_offset=roi_offset)
        para_1 = torch.real(beta)
        para_2 = torch.imag(beta)
    ...
    elif pulse_function=='excitation_alphabeta':
        alphaconj_beta,betanorm = cube.calculate_target_spindomain_excitation_v2(
    ...
    elif pulse_function=='excitation_v2':
        alphaconj_beta,betanorm = cube.calculate_target_spindomain_excitation_v2(
            flip=flip,phase=phase,roi=roi)
        para_1 = torch.real(alphaconj_beta)
        para_2 = torch.imag(alphaconj_beta)
